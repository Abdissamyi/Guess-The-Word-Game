...
document.querySelector("footer").innerHTML = `&copy; ${gameName} is created by Abdissamyi`;
...
function handleGuesses() {
    ...
    if (successGuess) {
        ...
        if (el) {
            ...
            const EnabeInputArray = Array.from(EnableInput) ;
            if(hintIndexes.length === 1){
                    EnabeInputArray[hintIndexes[0]].value = wordToGuess[hintIndexes[0]].toUpperCase();
            }
            if(hintIndexes.length === 2){
                    EnabeInputArray[hintIndexes[0]].value = wordToGuess[hintIndexes[0]].toUpperCase();
                    EnabeInputArray[hintIndexes[1]].value = wordToGuess[hintIndexes[1]].toUpperCase();
            }
        }
        else {
            // disabled the check button 
...
// function to click "check word" by the enter key
